...
// 获取模型详情
func FilterModelDetails(c *gin.Context) {
	var (
		err          error
		fieldDetails struct {
			model.Info
	...
	// 查询模型信息
	err = orm.Eloquent.Model(&model.Info{}).Where("identifies = ?", identifies).Find(&fieldDetails).Error
	if err != nil {
		app.Error(c, -1, err, "查询模型信息失败")
		return
	}
	...
	// 查询模型分组
	err = orm.Eloquent.Model(&model.FieldGroup{}).Where("info_id = ?", fieldDetails.Id).Find(&fieldDetails.FieldGroups).Error
	if err != nil {
		app.Error(c, -1, err, "查询模型信息失败")
		return
	}
	...
	for _, group := range fieldDetails.FieldGroups {
		err = orm.Eloquent.Model(&model.Fields{}).
			Where("info_id = ? and field_group_id = ?", fieldDetails.Id, group.Id).
			Find(&group.Fields).Error
		if err != nil {
			app.Error(c, -1, err, "查询字段列表失败")
			return
